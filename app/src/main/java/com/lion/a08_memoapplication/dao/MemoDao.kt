...
interface MemoDao {
    ...
    // 메모 제목으로 메모를 검색하는 메서드 -> 해당 내용을 포함하면 모두 출력할 수 있도록 한다
    @Query("""
        select * from MemoTable
        WHERE memoTitle LIKE '%' || :searchMemoTitle || '%'
        order by memoIdx desc
    """)
    fun selectMemoDataAllByMemoTitle(searchMemoTitle : String) : List<MemoVO>
}