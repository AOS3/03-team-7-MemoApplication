...
class SearchCategoryBottomSheetFragment(
    ...
    fun settingButton() {
        fragmentCategoryManagementBottomSheetBinding.apply {
            ...
            buttonCategoryManagementBottomSheetModify.setOnClickListener {
                ...
                val dialogCategoryManagementBinding =
                    DialogCategoryManagementBinding.inflate(layoutInflater)
                ...
                builder.setPositiveButton("삭제") { dialogInterface: DialogInterface, i: Int ->
                    CoroutineScope(Dispatchers.Main).launch {
                        ...
                        val work1 = async(Dispatchers.IO) {
                            MemoRepository.updateMemoCategoryIdx(
                                mainActivity,
                                selectedCategoryIdx,
                                basicCategoryIdx
                            )
                        ...
                        val work2 = async(Dispatchers.IO) {
                            CategoryRepository.deleteMemoDataByCategoryIdx(
                                mainActivity,
                                selectedCategoryIdx
                            )
                        }
                        ...
                        searchCategoryFragment.categoryList.removeAt(selectedPosition)
                        searchCategoryFragment.fragmentSearchCategoryBinding.recyclerViewSearchCategory.adapter?.notifyItemRemoved(
                            selectedPosition
                        )
                    }
                }
            ...
            buttonCategoryManagementBottomSheetDeleteCategoryWithMemos.setOnClickListener {
                ...
                builder.setPositiveButton("삭제") { dialogInterface: DialogInterface, i: Int ->
                    CoroutineScope(Dispatchers.Main).launch {
                        // 카테고리 내의 모든 메모를 삭제한다.
                        val work1 = async(Dispatchers.IO) {
                            MemoRepository.deleteMemoDataByCategoryIdx(
                                mainActivity,
                                selectedCategoryIdx
                            )
                        }
                        ...
                        mainActivity.addCategoryMenu()
                        searchCategoryFragment.categoryList.removeAt(selectedPosition)
                        searchCategoryFragment.fragmentSearchCategoryBinding
                            .recyclerViewSearchCategory.adapter?.notifyItemRemoved(selectedPosition)
                    }
