...
//    }
    ...
    fun settingToolbar(){
        fragmentCategoryManagementBinding.apply {
            ...
            // 검색 아이콘을 설정하고 누를 경우 카테고리 검색 화면으로 넘어가도록 한다
            toolbarCategoryManagement.inflateMenu(R.menu.toolbar_category_list_menu)
            toolbarCategoryManagement.setOnMenuItemClickListener {
                when(it.itemId){
                    R.id.memo_category_menu_search -> {
                        mainActivity.replaceFragment(FragmentName.SEARCH_CATEGORY_FRAGMENT,true,true,null)
                    }
                }
                true
            }
