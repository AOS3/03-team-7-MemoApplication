...
//    }
    ...
    fun settingToolbar(){
        fragmentShowMemoAllBinding.apply {
            ...
            // 검색 아이콘을 설정하고 누를 경우 검색 화면으로 넘어가도록 한다
            toolbarShowMemoAll.inflateMenu(R.menu.toolbar_memo_list_menu)
            toolbarShowMemoAll.setOnMenuItemClickListener {
                when(it.itemId){
                    R.id.memo_list_menu_search -> {
                        mainActivity.replaceFragment(FragmentName.SEARCH_MEMO_FRAGMENT,true,true,null)
                    }
                }
                true
            }
