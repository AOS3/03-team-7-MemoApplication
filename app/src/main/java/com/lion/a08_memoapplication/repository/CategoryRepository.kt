...
class CategoryRepository {
    companion object{
        ...
        // 카테고리 이름을 통해 카테고리 정보를 가져오는 메서드
        fun selectCategoryAllByCategoryName(context: Context, categoryName:String):MutableList<CategoryModel>{
            // 데이터를 가져온다
            val memoDataBase = MemoDataBase.getInstance(context)
            val dataList = memoDataBase?.categoryDao()?.selectCategoryDataByCategoryName(categoryName)
